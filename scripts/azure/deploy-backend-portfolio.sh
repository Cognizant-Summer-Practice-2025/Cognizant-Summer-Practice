...
IMAGE_TAG="${1:-latest}"
APP_NAME="backend-portfolio"
DOCKERFILE="$REPO_ROOT/backend/backend-portfolio/Dockerfile"
CONTEXT_DIR="$REPO_ROOT/backend/backend-portfolio"
...
PORTFOLIO_DB_HOST="${PORTFOLIO_DB_HOST:-}"
if [[ -z "$PORTFOLIO_DB_HOST" ]]; then
  PORTFOLIO_DB_HOST="$(az containerapp show -g "$AZ_RG" -n portfolio-db --query properties.configuration.ingress.fqdn -o tsv 2>/dev/null || true)"
fi
...
echo "🔍 Backend Portfolio Environment Configuration:"
