...
# Deploy backend-messages to Heroku
# Usage: ./deploy-backend-messages.sh [app-name]
...
# Default app name
DEFAULT_APP_NAME="cognizant-messages-backend"
...
echo -e "${BLUE}🚀 Deploying backend-messages to Heroku...${NC}"
...
# Navigate to backend-messages directory
cd "$(dirname "$0")/../../backend/backend-messages"
...
if [[ -n "${ConnectionStrings__Database_Messages:-}" ]]; then
    ...
    echo -e "${YELLOW}⚠️  ConnectionStrings__Database_Messages not found in .env${NC}"
fi
...
# Set email configuration from .env
if [[ -n "${Email__SmtpHost:-}" ]]; then
    echo -e "${BLUE}📧 Setting email configuration...${NC}"
    heroku config:set --app "$APP_NAME" \
    ...
    echo -e "${YELLOW}⚠️  Email configuration not found in .env${NC}"
    ...
    heroku config:set --app "$APP_NAME" \
        ...
        EMAIL__USESSL="true" \
        EMAIL__ENABLECONTACTNOTIFICATIONS="true" \
        EMAIL__TIMEOUTSECONDS="60" \
        EMAIL__MAXRETRYATTEMPTS="3" \
        EMAIL__RETRYDELAYSECONDS="5"
fi

# Set HTTP client configuration
heroku config:set --app "$APP_NAME" \
    HTTPCLIENT__TIMEOUT="00:00:30" \
    HTTPCLIENT__USERAGENT="MessagesService/1.0" \
...
echo -e "${GREEN}🎉 backend-messages successfully deployed to Heroku!${NC}"
