...
{
    /// <summary>
    /// OAuth2 Bearer token strategy for AI backend.
    /// </summary>
    public class OAuth2AuthenticationStrategy : IAuthenticationStrategy
    {
        ...
        public OAuth2AuthenticationStrategy(IUserAuthenticationService userAuthenticationService, ILogger<OAuth2AuthenticationStrategy> logger)
        {
        ...
        public async Task<ClaimsPrincipal?> AuthenticateAsync(HttpContext context)
        {
            try
            {
                ...
                _logger.LogInformation("Successfully validated token for user: {UserId}", principal.FindFirst(ClaimTypes.NameIdentifier)?.Value);
                return principal;
            }
            catch (Exception ex)
            {
