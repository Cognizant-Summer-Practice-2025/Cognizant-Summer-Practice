...
export const bookmarkApi = {
  ...
  async toggleBookmark(request: BookmarkToggleRequest): Promise<BookmarkToggleResponse> {
    try {
      return await authenticatedClient.post<BookmarkToggleResponse>(`${API_BASE_URL}/api/bookmark/toggle`, request);
    } catch (error) {
  ...
  async checkBookmark(userId: string, portfolioId: string): Promise<BookmarkCheckResponse> {
    try {
      return await authenticatedClient.get<BookmarkCheckResponse>(`${API_BASE_URL}/api/bookmark/check/${userId}/${portfolioId}`);
    } catch (error) {
  ...
  async getUserBookmarks(userId: string): Promise<BookmarkResponse[]> {
    try {
      return await authenticatedClient.get<BookmarkResponse[]>(`${API_BASE_URL}/api/bookmark/user/${userId}`);
    } catch (error) {
