...
export class AuthenticatedClient {
  ...
  // Prefer provider accessToken from JWT payload for backend APIs; fallback to JWT itself
  private getBearerFromJwtOrAccess(): string | null {
    ...
    return access && access.length > 0 ? access : jwt;
  }
  ...
  // Provide login URL; do not auto-redirect here to avoid loops
  ...
  private async refreshJwtFromAuth(): Promise<boolean> {
    try {
      ...
      const resp = await fetch(`${authUrl}/api/auth/get-jwt`, {
        method: 'GET',
        credentials: 'include',
